 ...
 */
public class AprioriMapper extends Mapper<ArrayWritable, ArrayWritable, ArrayWritable, ArrayWritable>{
	/**
	...
	public void map(ArrayWritable key, ArrayWritable value, Context context) throws IOException, InterruptedException {
		...
		for(MRTransaction tr : trx){
			if(tr.contains((Text[])key.toArray())){
				ss.add(tr);
			}
		}
		...
		if(ss.size()>0){
			...
			context.write(key, new ArrayWritable(MRTransaction.class, ss.toArray(new MRTransaction[ss.size()])));
		}
	}
